...
public class EtcdKVTest extends AbstractTest
{
    ...
    public void cleanUp() throws Exception
    {
        ...
        final ByteString endKey = ByteString.copyFrom("\0", "UTF-8");
        final GetOption option =
                GetOption.newBuilder()
                        .withSortField(RangeRequest.SortTarget.KEY)
                        .withSortOrder(RangeRequest.SortOrder.DESCEND)
                        .withRange(endKey).build();
        final ListenableFuture<RangeResponse> getFeature =
                kvClient.get(key, option);
        final RangeResponse response = getFeature.get();

        for (final KeyValue keyValue : response.getKvsList())
        {
    ...
    public void testPut() throws Exception
    {
        ...
        ListenableFuture<PutResponse> feature = kvClient
    ...
    public void testPutWithNotExistLease() throws Exception
    {
        ...
        ListenableFuture<PutResponse> feature = kvClient
                .put(sampleKey, sampleValue, option);
        try
        {
            PutResponse response = feature.get();
            test.assertTrue(response.hasHeader());
    ...
    public void testGet() throws Exception
    {
        ...
        {
            feature.get();
            ListenableFuture<RangeResponse> getFeature = kvClient
                    .get(sampleKey);
            RangeResponse response = getFeature.get();
            test.assertEquals(response.getKvsCount(), 1);
            test.assertEquals(response.getKvs(0).getValue()
                                      .toStringUtf8(), "sample_value2");
    ...
    public void testGetWithRev() throws Exception
    {
        ...
        ByteString sampleValue = ByteString.copyFrom("sample_value", "UTF-8");
        ByteString sampleValueTwo = ByteString
                .copyFrom("sample_value2", "UTF-8");
        ListenableFuture<PutResponse> feature = kvClient
                .put(sampleKey, sampleValue);
        try
        {
            ...
            GetOption option = GetOption.newBuilder()
                    .withRevision(putResp.getHeader().getRevision()).build();
            ListenableFuture<RangeResponse> getFeature = kvClient
                    .get(sampleKey, option);
            RangeResponse response = getFeature.get();
            test.assertEquals(response.getKvsCount(), 1);
    ...
    public void testGetSortedPrefix() throws Exception
    {
        ...
        try
        {
            ...
            test.assertEquals(response.getKvs(0).getKey()
                                      .toStringUtf8(), "test_key2");
            test.assertEquals(response.getKvs(0).getValue()
                                      .toStringUtf8(), "test_value");
            test.assertEquals(response.getKvs(1).getKey()
                                      .toStringUtf8(), "test_key1");
            test.assertEquals(response.getKvs(1).getValue()
                                      .toStringUtf8(), "test_value");
            test.assertEquals(response.getKvs(2).getKey()
                                      .toStringUtf8(), "test_key0");
            test.assertEquals(response.getKvs(2).getValue()
                                      .toStringUtf8(), "test_value");
        }
        catch (Exception e)
        {
    ...
    public void testDelete() throws Exception
    {
        ...
        try
        {
            ...
            // delete the keys
            ListenableFuture<DeleteRangeResponse> deleteFuture = kvClient
                    .delete(keyToDelete);
    ...
    public void testTxn() throws Exception
    {
        ...
        try
        {
            ...
            Cmp cmp = new Cmp(sampleKey, Cmp.Op.GREATER, CmpTarget
                    .value(cmpValue));
            Txn txn = Txn.newBuilder().If(cmp)
            ...
            test.assertEquals(getResp.getKvs(0).getValue()
                                      .toStringUtf8(), "XYZ");
        }
        catch (Exception e)
        {
