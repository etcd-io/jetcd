...
public class ClientServiceChecks extends TestSupport {
    ...
    public Option[] config() {
        final MavenArtifactUrlReference karafUrl = maven().groupId("org.apache.karaf").artifactId("apache-karaf-minimal")
            .versionAsInProject().type("tar.gz");

        final MavenUrlReference karafStandardRepo = maven().groupId("org.apache.karaf.features").artifactId("standard")
            .version(getKarafVersion()).classifier("features").type("xml");

        return new Option[] { systemProperty("org.ops4j.pax.logging.DefaultServiceLog.level").value("WARN"),
                ...
                configureSecurity().disableKarafMBeanServerBuilder(), configureConsole().ignoreLocalConsole(),
                features(karafStandardRepo, "scr", "wrap"), features(getFeaturesFile().toURI().toString(), "etcd-io-jetcd"),
                mavenBundle().groupId("org.assertj").artifactId("assertj-core").versionAsInProject().start(),
                editConfigurationFilePut("etc/io.etcd.jetcd.cfg", "endpoints", PaxExamWrapperTest.getClientEndpoints()),
                editConfigurationFilePut("etc/io.etcd.jetcd.resolver.dnssrv.cfg", "foo", "bar"), keepRuntimeFolder(),
                cleanCaches(), logLevel(LogLevelOption.LogLevel.INFO)
        };
    }
